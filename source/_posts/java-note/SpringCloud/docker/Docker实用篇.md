...
- docker是一个**系统进程**；虚拟机是在操作系统中的操作系统
...
**镜像（Image）**：Docker将应用程序及其所需的依赖、函数库、环境、配置等**文件打包**在一起，称为镜像。
...
```shell
[root@VM-0-5-centos ~]# docker load -i nginx.tar
...
![image-20210830204232530](Docker实用篇/image-20210830204232530.png)
...
**在官方镜像中都有介绍How to use this image。**
...
8c282135b883   nginx     "/docker-entrypoint.…"   41 seconds ago   Up 41 seconds   0.0.0.0:80->80/tcp, :::80->80/tcp   mn
```
...
![image-20210830210153478](Docker实用篇/image-20210830210153478.png)
...
**查看DockerHub网站中的nginx页面**(Hosting some simple static content)，可以知道nginx的html目录位置在`/usr/share/nginx/html`
...
```shell
root@8c282135b883:/usr/share/nginx/html# ls
50x.html  index.html
```
...
[root@VM-0-5-centos ~]# docker ps
CONTAINER ID   IMAGE     COMMAND                  CREATED          STATUS          PORTS                                       NAMES
3721e348e496   redis     "docker-entrypoint.s…"   18 seconds ago   Up 17 seconds   0.0.0.0:6379->6379/tcp, :::6379->6379/tcp   redis
```
...
```
[root@VM-0-5-centos ~]# docker exec -it redis redis-cli
127.0.0.1:6379> 
```
...
```
root@3721e348e496:/data# redis-cli
127.0.0.1:6379> 
```
...
![image-20210830211319387](Docker实用篇/image-20210830211319387.png)
...
```shell
[root@VM-0-5-centos ~]# docker volume --help 
...
DRIVER    VOLUME NAME
local     5511e44a3bb854589933ef3f2096ce5a7df5ec43b2f7279a036cb6af723c1ce4
local     html
```
...
```
[root@VM-0-5-centos ~]# docker volume inspect html
[
    {
        "CreatedAt": "2021-08-31T11:07:21+08:00",
        "Driver": "local",
        "Labels": {},
...
我们在创建容器时，可以通过 **-v 参数**来挂载一个数据卷到某个容器内目录，命令格式如下：
...
注：如果数据卷html不存在，docker会去创建一个

![image-20210831112617567](Docker实用篇/image-20210831112617567.png)
...
容器不仅仅可以挂载数据卷，也可以**直接挂载到宿主机目录上**。关联关系如下：
...
```bash
[root@VM-0-5-centos tmp]# mkdir --help 
...
![image-20210831153403814](Docker实用篇/image-20210831153403814.png)
...
- 步骤2：拷贝课前资料中的docker-demo.jar文件到docker-demo这个目录
  ...
  ```shell
  [root@VM-0-5-centos docker-demo]# ll
  total 96896
  -rw-r--r-- 1 root root 25620395 Aug 31 15:41 docker-demo.jar
  -rw-r--r-- 1 root root      494 Aug 31 15:40 Dockerfile
  -rw-r--r-- 1 root root 73596928 Aug 31 15:41 jdk8.tar.gz
  ```
...
1. Dockerfile的本质是一个**文件**，通过指令描述镜像的构建过程
...
Compose文件是一个**文本文件**，通过指令定义集群中的每个容器如何运行。格式如下：
...
![image-20210831160426702](Docker实用篇/image-20210831160426702.png)
...
② 修改自己的cloud-demo项目，将**数据库、nacos地址都命名为docker-compose中的服务名**
...
查看日志，发现userservice服务未发现nacos

```bash
[root@localhost cloud-demo]# docker-compose logs -f

userservice_1   | com.alibaba.nacos.api.exception.NacosException: failed to req API:/nacos/v1/ns/instance/list after all servers([nacos:8848]) tried: java.net.ConnectException: Connection refused (Connection refused)

```

原因是服务建立时，nacos还未安装好，重启服务即可
...
用浏览器访问http://192.168.194.128:10010/user/1?authorization=admin正常
...
① 重新tag本地镜像，名称前缀为私有仓库的地址：192.168.150.101:8080/
 ...
 ```sh
docker tag nginx:latest 192.168.194.128:8080/nginx:1.0 
 ```
...
```sh
docker push 192.168.194.128:8080/nginx:1.0
```
...
```sh
docker pull 192.168.194.128:8080/nginx:1.0 
```
