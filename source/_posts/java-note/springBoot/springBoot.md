...
java/resources都是内路径的根路径（classpath）
...
**==编写一个配置类（@Configuration），是WebMvcConfigurerAdapter（@deprecate）类型；不能标注@EnableWebMvc==**;

实现接口WebMvcConfigurer为2支持
...
thymeleaf默认为classpath:/templates/xxx.html
...
// redirect: 表示重定向到一个地址  /代表当前项目路径// forward: 表示转发到一个地址
...
springboot在WebMvcAutoConfiguration.java中默认配置的格式是
...
​	2、（html）页面上能用的数据，或者是json返回能用的数据都是通过errorAttributes.getErrorAttributes得到；
...
//2.0以上版本===============
 @Bean //一定要将这个定制器加入到容器中
public WebServerFactoryCustomizer<ConfigurableServletWebServerFactory> aa(){
...
![](E:\pictures\Language\spring boot\images\configurable继承.png)
...
**WebServerFactoryCustomizerBeanPostProcessor**（2.0版本）
...
```java
private void createWebServer()//2.0版本
```
...
​	从ioc容器中获取EmbeddedServletContainerFactory 组件；**TomcatEmbeddedServletContainerFactory**(TomcatServletWebServerFactory)创建对象，后置处理器一看是这个对象，就获取所有的定制器来先定制Servlet容器的相关配置；
...
this.onRefresh(); //	创建servlet容器
this.registerListeners();
this.finishBeanFactoryInitialization(beanFactory); //创建我们自己定义的bean组件
```
...
![](E:\pictures\Language\spring boot\images\ServletContainerInitializer目录.png)
...
```java
//自己的SpringBootServletInitializer的子类重写了configure方法
public class ServletInitializer extends SpringBootServletInitializer {
